...
export default async (request: Request, _context: Context) => {
  ...
  try {
    ...
    // Build the context block — everything the AI needs to know
    ...
    // Build the notes block — prioritized order
    let notesBlock = '';
    ...
    // Progression hints from the client-side engine
    let extraContext = '';
...
- No preamble, no sign-off, no encouragement, no "Great class!" fluff.`;
    ...
    // Strip any markdown artifacts that Haiku might produce
