  ...
  return (
    <div className="flex items-center justify-between py-2">
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => onChange(Math.max(min, value - 0.5))} className="w-8 h-8 rounded-full bg-[var(--surface-card)] text-[var(--text-primary)] font-bold">-</button>
        <span className="type-body font-semibold text-[var(--text-primary)] w-12 text-center">{value}{suffix}</span>
        <button onClick={() => onChange(Math.min(max, value + 0.5))} className="w-8 h-8 rounded-full bg-[var(--surface-card)] text-[var(--text-primary)] font-bold">+</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="page-container pb-24">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="type-display text-[var(--text-primary)]">Wellness Settings</h1>
      </div>

      {/* Medication Section */}
      <section className="mb-8">
        <h2 className="type-caption font-semibold text-[var(--text-muted)] uppercase tracking-wide mb-4">Medication</h2>
        <div className="bg-[var(--surface-card)] rounded-xl p-4 space-y-4">
          {/* Med Type */}
          <div>
            <p className="type-body text-[var(--text-secondary)] mb-2">Med Type</p>
          ...
          {/* Daily Doses */}
          <div>
            <p className="type-body text-[var(--text-secondary)] mb-2">Daily Doses</p>
            <div className="flex gap-2">
              {([2, 3] as const).map(n => (
                <button
                  key={n}
            ...
            {medConfig.medType === 'IR' ? (
              <>
                ...
                <div className="border-t border-[var(--border-subtle)] pt-2 mt-2">
                  <p className="type-body text-[var(--text-secondary)] mb-1">Dose 2 Window</p>
                ...
                {medConfig.maxDoses === 3 && (
                  <div className="border-t border-[var(--border-subtle)] pt-2 mt-2">
                    <p className="type-body text-[var(--text-secondary)] mb-1">Dose 3 Window</p>
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="type-caption font-semibold text-[var(--text-muted)] uppercase tracking-wide">Wellness Checklist</h2>
          <button
        ...
        {SECTIONS.map(section => {
          ...
          return (
            <div key={section} className="mb-4">
              <p className="type-body text-[var(--text-secondary)] mb-2">{SECTION_LABELS[section]}</p>
              <div className="bg-[var(--surface-card)] rounded-xl overflow-hidden">
                {sectionItems.map(item => {
                  const IconComp = ICON_MAP[item.icon] || Sparkles;
                  return (
                    <div key={item.id} className="flex items-center gap-3 px-4 py-3 border-b border-[var(--border-subtle)] last:border-b-0">
                      <button
